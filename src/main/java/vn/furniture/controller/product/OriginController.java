...
@WebServlet(name = "OriginController", value = "/origin")
public class OriginController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int amount = productDAO.getAmountOriginId(oid);
        int endPage = amount / limit;
        if (amount % limit != 0) {
            endPage++;
        }
        request.setAttribute("originPage", endPage);
        ...
        List<Product> products = productDAO.getListWithOriginId(oid, index, limit);
